...
**Status:** drafted 2026-05-11 after deep-dive competitive analysis against LOPs ([dotsimulate](https://docs.dotsimulate.com/getting-started/)), TWOZERO ([twozero.ai](https://twozero.ai/)), [bottobot/touchdesigner-mcp-server](https://github.com/bottobot/touchdesigner-mcp-server), and [8beeeaaat/touchdesigner-mcp](https://github.com/8beeeaaat/touchdesigner-mcp).
...
The chat feels less like a remote tool and more like a co-pilot embedded in the network — closer in feel to LOPs's operator-native model, but with our open-source stack, cost wins, and broader feature mix.

---

## Competitive context (as of May 2026)

| Product | Architecture | Standout features |
|---|---|---|
| **dpsk4 (this repo)** | Two .tox files (MCP server + standalone chat-pipe) | 103+91 tools, subagents, recipes, memory, skills, safety, hint injection, **DeepSeek prefix cache (50× discount)**, parallel dispatch, vision streaming, patch sessions |
| **[LOPs (dotsimulate)](https://docs.dotsimulate.com/getting-started/)** | **Agents as native TD operator family** — Controllers (20+), Modifiers (15+), Pipelines, Retrievers, Settings. SideCar processes for heavy work. | **External Op Tools sequence**, SideCar isolation, STT/TTS/VAD/image-gen pipelines, **graph-based memory**, swarms + scheduling, **Reverse-MCP**, ActivityViz, local model support (Ollama/LM Studio/llama.cpp) |
| **[TWOZERO (404.zero + setupdesign)](https://twozero.ai/)** | Backend SaaS — server-side processing, streams to TD via MCP (Cursor + Claude Code, Claude Opus 4.6) | **Production-tested curated templates** (15-yr touring credibility), **auto-troubleshooting**, **auto-docs**, live patch editing during performance, GPU/latency metrics in UI, beat detection + audio sync |
| **[bottobot/touchdesigner-mcp-server](https://github.com/bottobot/touchdesigner-mcp-server)** | Pure docs MCP, no live control | 630 operators + 14 tutorials + 69 Python classes + 16 GLSL patterns + version-aware filtering |
| **[8beeeaaat/touchdesigner-mcp](https://github.com/8beeeaaat/touchdesigner-mcp)** | Simple CRUD MCP | Minimal, ja-jp localized |

**Strategic implications:**
- LOPs is the architectural threat (operator-native + SideCar + voice).
- TWOZERO is the polished-product threat (curated content + auto-troubleshoot + auto-explain).
- Neither competitor mentions DeepSeek prefix-cache discipline — our cost moat.
- bottobot's docs depth is still a content gap.
- Open-source positioning is our asymmetric advantage.
...
**Goal:** cover TD's #1 use case (audio-reactive visuals) and #2 use case (custom shaders) end-to-end. Provide curated templates that match what LOPs / TWOZERO ship.
...
**Goal:** the agent feels like a senior pair-programmer who already knows the project. Match TWOZERO's auto-explain + auto-troubleshoot. Match LOPs's sandboxed-execution model.

### Features

| # | Feature | Description | Default state |
|---|---|---|---|
| 5.1 | **Auto-explain project** (match TWOZERO) | New tool `td_explain_project(path="/project1", depth=3)`. Surveys nodes under path → identifies signal flow → classifies COMPs by role → returns Markdown writeup with section headers. Uses existing primitives. | `enabled` |
| 5.2 | **Auto-troubleshoot** (match TWOZERO) | New tool `td_audit_and_fix(path, dry_run=True)`. Scans errors recursively → groups by root cause → proposes a fix plan with snapshots → if `dry_run=False`, applies with user confirmation per group. | `enabled` |
...
**Goal:** parity with LOPs's depth. Out-of-process workers. User-defined tools as TD operators. Graph memory. Reverse-MCP.

### Features (research-first; spike each before committing)

| # | Feature | Description | Risk |
|---|---|---|---|
| 6.1 | **SideCar process** for heavy work (vision, embeddings, STT, image-gen) | Out-of-process Python worker. IPC over local Unix socket / Windows pipe. Heavy LLM-adjacent work (e.g. screenshot analysis with a vision model, embedding generation, Whisper STT) runs outside TD's process. | High — new IPC layer, process lifecycle, error propagation |
| 6.2 | **External Op Tools** | Let any TD operator declare itself as an agent tool via a contract (e.g. a custom-parameter page with name "TDPilotTool" + role / args / handler attached as a custom Python class). Agent's tool registry scans the network at runtime + auto-loads. | High — composition pattern needs careful spec |
...
| 6.4 | **Reverse-MCP** | Let external agents (Cursor, Claude Code) drive the chat-pipe. Add an MCP-server mode that wraps the agent loop as MCP tools. The chat-pipe webserver could serve both HTML chat AND MCP-over-WS simultaneously. | Medium |
| 6.5 | **Local model support** | Configurable base URL + auth pattern that accepts Ollama / LM Studio / llama.cpp endpoints. Match LOPs's "local-first" optionality. | Low — mostly config + docs |
...
### Effort
**8-10 weeks.** Each feature is multi-week. Plan: 4 weeks SideCar + External Op Tools (the two LOPs-parity features), 3 weeks graph memory + reverse-MCP, 1-2 weeks codebase reorg + local model support.

### Tox-rebuild requirement
**Both tox files**, multiple times during the phase as the codebase reorganizes.

### Recommendation
Do NOT commit to all of Phase 6 in one release. Spike SideCar with a single workload (e.g. STT via Whisper.cpp) first; ship if it works; expand. Same for External Op Tools — spike with one user-defined tool type.
...
FLAGS = {
    ...
    "SIDECAR_VISION":        {"default": False, "since": "3.0.0"},
    # ...
}
...
| v3.0.0 | Phase 6 — Architecture evolution | weeks 14-23 | SideCar, External Op Tools, graph memory, reverse-MCP, local models |
