...
**Authored:** 2026-05-18 (post v2.4.0 ship + dreamrec/TDPilot v1.6.16 upstream audit + dotsimulate LOPs 0.3.0 review).
...
- **Source of v2.5 themes:** [`docs/plans/v2.4_IMPLEMENTATION_PLAN.md`](./v2.4_IMPLEMENTATION_PLAN.md) + upstream `dreamrec/TDPilot` v1.6.16 (2026-05-18) + dotsimulate LOPs 0.3.0 (2026-05-08).
...
7. **GitHub Release** — after `git push origin v2.5.0`, also run `gh release create v2.5.0`. Per [feedback memory](file:///Users/visansilviugeorge/.claude/projects/-Users-visansilviugeorge-Desktop-DREAM-AI-TDPilot-deepseekv4/memory/feedback_create_github_release_after_tag.md).
...
|---|---|---|---|---|
| `v2.5.1-activity-log` | Activity log ring buffer + `td_get_activity_log` + `_read_journal` hints | 3 days | Both | not_started |
| `v2.5.2-ocr` | OCR sidecar on Phase B vision pipeline (`[ocr]` extras) | 4 days | MCP only | not_started |
| `v2.5.3-tool-approval` | `Approvalmode` COMP param + runtime gate + chat UI | 3 days | API only | not_started |
| `v2.5.4-auth-fallback` | TD-side MCP auth fallback file (`~/.tdpilot-dpsk4/.tdpilot-dpsk4.env` enforcement) | 1 day | None | not_started |
| `v2.5.5-td-release-card` | TD 2025.32820 release card markdown | 1 hour | None | not_started |
| `v2.5.6-npm-stdio` | npm wrapper stdout/stderr separation (Claude Desktop compat) | 1 day | None | not_started |
| `v2.5.7-check-updates` | `td_check_for_updates` read-only tool | 2 days | MCP only | not_started |
| `v2.5.8-log-receiver` | (Stretch) MCP-stderr tail to TD Table DAT | 2 days | API only | not_started |
...
### Engine decision
**PaddleOCR** (production-grade, CPU+GPU, multilingual, well-maintained). Optional via pyproject extras: `pip install tdpilot-dpsk4[ocr]`. Subprocess sidecar pattern (mirrors dotsimulate's Sidecar) so the 400MB model doesn't bloat the MCP server process.
...
### Why
v2.4 added the **User-Intent Gate** (B-008 Bug 8 fix) at system-prompt level. That's first-line defense. Tool Approval adds a runtime gate: even with valid auth + intent, destructive tools require explicit click-through. Layered defense. Closes the residual concern from [project_tdpilot_api_authmode_open_default.md memory](file:///Users/visansilviugeorge/.claude/projects/-Users-visansilviugeorge-Desktop-DREAM-AI-TDPilot-deepseekv4/memory/project_tdpilot_api_authmode_open_default.md) about drive-by-RCE risk in `Authmode=open`.
...
### Why
The dpsk4 fork already cites `~/.tdpilot-dpsk4/.tdpilot-dpsk4.env` in AGENTS.md naming pins, but the MCP-server auth code may still rely on env vars only (verify). Closes the cold-start auth-401 scenario from [feedback_td_mcp_auth.md memory](file:///Users/visansilviugeorge/.claude/projects/-Users-visansilviugeorge-Desktop-DREAM-AI-TDPilot-deepseekv4/memory/reference_td_mcp_auth.md).
...
### Why
TD 2025.32820 (May 2026) added Trace/Triangulate POP, DMX POP pipeline, Layer Mix TOP, RTX Video TOP, ST2110 I/O, color overhaul. Content already lives in [project_td_2025_32820_features.md memory](file:///Users/visansilviugeorge/.claude/projects/-Users-visansilviugeorge-Desktop-DREAM-AI-TDPilot-deepseekv4/memory/project_td_2025_32820_features.md). Materialize as a queryable knowledge card.
...
### Why
dotsimulate LOPs ships a Log Receiver operator. Tail MCP-server stderr (and chat-pipe internal logs) into a TD Table DAT for in-app debugging. Multiplies the value of activity_log + journal_hints.
...
Both `.tox` files need rebuild. Canonical Textport recipe in [feedback_td_tox_rebuild_recipe.md memory](file:///Users/visansilviugeorge/.claude/projects/-Users-visansilviugeorge-Desktop-DREAM-AI-TDPilot-deepseekv4/memory/feedback_td_tox_rebuild_recipe.md). Auto-mirror (v2.2.1+) syncs to main repo.
...
### npm publish
Trusted Publisher OIDC handles this automatically on tag push via `.github/workflows/release-assets.yml`. **Do NOT add NPM_TOKEN** — per [feedback_npm_trusted_publisher_oidc.md memory](file:///Users/visansilviugeorge/.claude/projects/-Users-visansilviugeorge-Desktop-DREAM-AI-TDPilot-deepseekv4/memory/feedback_npm_trusted_publisher_oidc.md).
