...
Web ingestion completes the trio: with hybrid retrieval working, we can ingest external docs (official TD pages, POPX examples, third-party references) into the knowledge corpus on demand.
...
### Why
With hybrid retrieval live (v2.6.1), the knowledge corpus is queryable. The agent should be able to grow it on demand — ingest official TD docs, POPx examples, general references. Closes the corpus-completeness gap that motivates many "I don't know that operator" turn failures.
