...
_RECOVERY_HINTS: tuple[tuple[re.Pattern[str], str], ...] = (
    ...
    # (184 messages, 11 tool_result errors). Each one is a wrong-API
    # guess for a real concept; the hint points at the right TD API.
    # ------------------------------------------------------------------
    (
    ...
    (
        re.compile(
            r"'td\.renderTOP' object has no attribute "
            r"'(cooking|numCooks|xres|yres)'",
            re.IGNORECASE,
        ),
        (
